...
public class TimetableMain extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		db=new DataBase(TimetableMain.this);
...
//			finish();			
		...
		//用适配器为各选项卡添加所要显示的内容
		for(int i=0;i<7;i++){
			cursor[i]=TimetableMain.db.select(i);		
		...
		exitButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
			    ...
			    final AlertDialog.Builder builder = new AlertDialog.Builder(TimetableMain.this);
				exit(builder);
			}
		}); 
		...
		setButton.setOnClickListener(new OnClickListener() {
			...
			@Override
			public void onClick(View v) {
				Intent intent = new Intent(TimetableMain.this, SetActivity.class);
					...
					if((tv.getText()).toString().equals("")){
						...
						builder.setItems(R.array.edit_options1, new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {					
								...
								if(which == 0){					
									new MyDialog(TimetableMain.this).add(currentDay,n);
								}
							}
						});
						builder.create().show();
					...
					//否则启动修改对话框，或直接删除数据
						...
						builder.setItems(R.array.edit_options2, new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface dialog, int which) {
								//如果单击了该列表项，则跳转到编辑课程信息的界面
								if(which == 0){					
									new MyDialog(TimetableMain.this).modify(currentDay,n);
								}
								if(which == 1){
									...
									switch(n2){
										case 0:
											for(int m=0;m<n1;m++){
												TimetableMain.db.deleteData(currentDay,n+m+1);
												}
											break;
	
										case 1:
											TimetableMain.db.deleteData(currentDay,n);
											for(int m=1;m<n1;m++){
												TimetableMain.db.deleteData(currentDay,n+m);
												}
											break;		
										case 2:
											TimetableMain.db.deleteData(currentDay,n-1);
											TimetableMain.db.deleteData(currentDay,n);
											for(int m=2;m<n1;m++){
												TimetableMain.db.deleteData(currentDay,n+m-1);
												}
